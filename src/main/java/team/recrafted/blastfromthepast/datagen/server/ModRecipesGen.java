...
public class ModRecipesGen extends RecipeProvider {
    ...
    protected void buildRecipes(@NotNull Consumer<FinishedRecipe> recipeOutput) {
        ...
        SimpleCookingRecipeBuilder.smelting(Ingredient.of(ModBlocks.BEAST_CHOPS.get().asItem()), RecipeCategory.FOOD, ModBlocks.BEAST_CHOPS_COOKED.get().asItem(), 0.35F, 200).unlockedBy(getHasName(ModBlocks.BEAST_CHOPS.get().asItem()), has(ModBlocks.BEAST_CHOPS.get().asItem()))
                .save(recipeOutput, ResourceLocation.fromNamespaceAndPath(BlastFromThePast.MOD_ID, "smelting_beast_chops"));
        simpleCookingRecipe(recipeOutput, "smoking", RecipeSerializer.SMOKING_RECIPE, 100, ModBlocks.BEAST_CHOPS.get(), ModBlocks.BEAST_CHOPS_COOKED.get(), 0.35F);
        ...
        twoByTwoPacker(recipeOutput,RecipeCategory.BUILDING_BLOCKS, ModBlocks.SNOW_BRICK.BLOCK.get(), Items.SNOW_BLOCK);
        blockGroup(recipeOutput, ModBlocks.SNOW_BRICK);
        twoByTwoPacker(recipeOutput,RecipeCategory.BUILDING_BLOCKS, ModBlocks.ICE_BRICK.BLOCK.get(), Items.ICE);
        ...
        createAntlerDisplay(recipeOutput, ModBlocks.ANTLER_DISPLAY.get(), ModItems.STRAIGHT_GLACEROS_ANTLERS.get());

        createAntlerDisplay(recipeOutput, ModBlocks.BROAD_ANTLER_DISPLAY.get(), ModItems.BROAD_GLACEROS_ANTLERS.get());

        createAntlerDisplay(recipeOutput, ModBlocks.CURLY_ANTLER_DISPLAY.get(), ModItems.CURLY_GLACEROS_ANTLERS.get());

        createAntlerDisplay(recipeOutput, ModBlocks.SPIKEY_ANTLER_DISPLAY.get(), ModItems.SPIKEY_GLACEROS_ANTLERS.get());

        ShapedRecipeBuilder.shaped(RecipeCategory.COMBAT, ModItems.BEAR_GLOVES.get())
                .define('L', Items.LEATHER)
                .define('C', ModItems.BEAR_CLAW.get())
                .pattern(" C ")
                .pattern("CLC")
                .pattern("LLL")
                .unlockedBy("has_bear_claw", has(ModItems.BEAR_CLAW.get()))
                .save(recipeOutput);
    }

    private void stoneGroup(Consumer<FinishedRecipe> pRecipeOutput, BFTPStoneGroup group) {
        smeltingResultFromBase(pRecipeOutput, group.STONE.get().asItem(), group.COBBLESTONE.get().asItem());
    ...
    private void woodGroup(Consumer<FinishedRecipe> pRecipeOutput, BFTPWoodGroup group) {
        ...
        woodFromLogs(pRecipeOutput, group.WOOD.get(), group.LOG.get());
        woodFromLogs(pRecipeOutput, group.STRIPPED_WOOD.get(), group.STRIPPED_LOG.get());
    ...
    private static void createAntlerDisplay(Consumer<FinishedRecipe> recipeOutput, ItemLike display, ItemLike antler) {
        ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, display)
                .define('L', Items.LEATHER)
                ...
                .pattern("WWW")
                .pattern("WWW")
                .unlockedBy("has_" + BuiltInRegistries.ITEM.getKey(antler.asItem()).getPath(), has(antler))
                .save(recipeOutput);
    }
}
