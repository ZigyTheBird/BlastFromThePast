...
public class BurrelRandomStrollGoal extends WaterAvoidingRandomStrollGoal {
    ...
    @Override
    public void start() {
        this.mob.getNavigation().moveTo(this.wantedX, this.wantedY, this.wantedZ, onTree ? this.speedModifier : 0.5F);
    ...
    protected Vec3 getPosition() {
        ...
        onTree = !this.burrel.wantsToBeOnGround() && onTree(burrel.blockPosition());
        if (onTree) {
            if (cooldown > 0) {
                cooldown--;
                return null;
            ...
            if (pos == null || pos.distanceTo(this.burrel.position()) < 2 || !onTree(new BlockPos((int) pos.x(), (int) pos.y(), (int) pos.z()))) return null;
